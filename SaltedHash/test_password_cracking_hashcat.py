...
def run_cracking_test(password_file, use_salt=True, attack_mode="dictionary"):
    ...
    log_file = f"{OUTPUT_DIR}/{base_filename}_{salt_suffix}_{attack_suffix}_log.txt"
    ...
    # Create a log file for hashcat output
    with open(log_file, 'w') as log:
        if attack_mode == "dictionary":
            ...
            # Add status monitor with more frequent updates
            ...
            for line in iter(process.stdout.readline, ''):
                ...
                if "Speed" in line:
                    ...
                    if match:
                        ...
                        # Calculate total attempts so far (time × speed)
                        current_attempt += last_speed
                        attempts.append(current_attempt)
                
                # Also capture progress from "Recovered" lines
                elif "Recovered" in line and "Progress" not in line and "/" in line:
                    match = re.search(r'Recovered\.+: (\d+)/(\d+)', line)
                    if match:
                        cracked = int(match.group(1))
                        total = int(match.group(2))
                        
                        # Record data point if not already captured
                        if len(times) == 0 or times[-1] != elapsed:
                            times.append(elapsed)
                            cracked_counts.append(cracked)
                            
                            # Calculate total attempts (time × speed)
                            current_attempt = elapsed * last_speed
                            attempts.append(current_attempt)
                
                # Also check for completion lines
                elif "All hashes have been recovered" in line or "Exhausted" in line:
                    if os.path.exists(result_file):
                        cracked = sum(1 for _ in open(result_file))
                        
                        # Add final data point if needed
                        if len(times) == 0 or cracked_counts[-1] != cracked:
                            times.append(elapsed)
                            cracked_counts.append(cracked)
                            current_attempt = elapsed * last_speed
                            attempts.append(current_attempt)
            ...
            # Run brute force for lengths 4 (since we're only generating length 4 passwords)
            length = 4
            ...
            if os.path.exists(result_file):
                total_cracked = sum(1 for _ in open(result_file))
    ...
    # If we didn't collect any data points, add at least one
    if len(times) == 0 and cracked > 0:
        times = [elapsed]
        cracked_counts = [cracked]
        attempts = [elapsed * last_speed if last_speed > 0 else total_passwords]  # Estimate attempts
    
    # Make sure to add the final state if not already added
    if len(times) > 0 and cracked_counts[-1] != cracked:
        times.append(elapsed)
        cracked_counts.append(cracked)
        attempts.append(elapsed * last_speed if last_speed > 0 else attempts[-1] + total_passwords)
    
    # Save raw data for plotting - ensure we have at least one data point
    if len(times) > 0:
        df = pd.DataFrame({
            'Time': times,
        ...
        print(f"Saved {len(times)} data points to {stats_file}")
    else:
        # If no data was collected, create a dummy entry
        df = pd.DataFrame({
            'Time': [elapsed],
            'Cracked': [cracked],
            'Attempts': [total_passwords]  # Rough estimate
        })
        df.to_csv(stats_file, index=False)
        print(f"No progress data collected, saved final result to {stats_file}")
